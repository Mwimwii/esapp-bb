...
export async function importHubImages(hub: hubspot.Client, s3: S3Client, manager: EntityManager) {
    ...
    do {
        ...
        engagementResult.body.results.filter(r => r.attachments.length > 0 || (r.engagement.bodyPreviewHtml && r.engagement.bodyPreviewHtml.includes('/filemanager/api/v2/files'))).forEach(result => {
